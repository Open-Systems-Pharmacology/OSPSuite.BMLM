...
|-------------------------|----------------------------------------------------------------------------------------------------------------------|
| **Name**                | The identifier and display name of the parameter in reports.  Must be unique                                                     |
...
ToDO when to use a factor and potential problem with export.
Use if you want to fit the relative change of distributed parameter like bloodflows or organ volumes over different individuals or
to fit the relative change of a group of similar parameter together. 
Attention Parameter which are flagged as factor are not exported by `exportModelParametersToConfigTables` of an `BMLMOptimization` object as the generation of populations and individuals with configuration tables is based on absolute values.
