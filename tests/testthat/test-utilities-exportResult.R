# setup functions loads testproject with variable projectConfiguration
test_that("exportPopulationWithVariability creates new population file", {
  ...
  # Skip the first columns (usually IndividualId or similar)
  paramNames <- utils::tail(names(dtOriginalPop), 4)
  ...
  testVarData <- data.table::data.table(
    ...
    `parameter Group` = paste0("Group_", ceiling(seq_along(paramNames) / 2)),
    distribution = "norm",
    p1_type = "mean",
    p1_value = 1.0,
    p2_type = "sd",
  ...
  wbPop <- xlsxAddDataUsingTemplate(
    wb = wbPop,
...
test_that("exportPopulationWithVariability handles missing files gracefully", {
  ...
  expect_error(
    suppressMessages(exportPopulationWithVariability(
      projectConfiguration = projectConfiguration,
